...
# Submodule Profile: traces each declared submodule independently for low-VRAM edge-device execution
# (a distinct entry point -- SubmoduleExportSpec / export_submodules -- not a `profile=` string, below)
```
...
### 6.3 The Submodule-Export Path (`SubmoduleExportSpec`)
* **Core Philosophy**: Highly optimized, low-VRAM edge-device execution, and the only split mechanism this
  compiler offers (an earlier `profile="atomic"` path that inferred the split by partitioning one flattened
  trace on scope metadata was retired -- EXPORT-ROADMAP.md R7 -- after repeated scope mis-attribution bugs).
* **Declared, not inferred**: the caller declares the boundary (`SubmoduleExportSpec`: prefix/aux/per-layer/
  suffix), and each piece is traced *independently* via its own real captured forward call rather than
  reconstructed from one flattened trace after the fact. Repeated blocks (e.g. decoder layers) are then
  re-derived structurally (`find_repeated_blocks`), so a wrong claim about the boundary raises immediately
  during export instead of silently producing a working-looking but wrong model.
* **No silent fallback**: unlike the retired atomic path, there is no monolithic downgrade-on-failure --a
  submodule export either succeeds against the declared boundary or raises.
