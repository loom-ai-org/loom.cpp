...
BiLstmStepper::BiLstmStepper(GgufModel& model, GraphTopology fwd_h_topo, GraphTopology fwd_c_topo,
    ...
    fwd_h_builder_ = std::make_unique<GraphBuilder>(fwd_h_topo_, model_, backends_, /*kv_cache=*/nullptr);
    fwd_c_builder_ = std::make_unique<GraphBuilder>(fwd_c_topo_, model_, backends_, /*kv_cache=*/nullptr);
    bwd_h_builder_ = std::make_unique<GraphBuilder>(bwd_h_topo_, model_, backends_, /*kv_cache=*/nullptr);
    bwd_c_builder_ = std::make_unique<GraphBuilder>(bwd_c_topo_, model_, backends_, /*kv_cache=*/nullptr);
}
...
std::vector<std::vector<float>> BiLstmStepper::run(const std::vector<std::vector<float>>& sequence) {
    ...
    for (uint32_t t = 0; t < T; ++t) {
        std::vector<float> h = lstm_cell_step(*fwd_h_builder_, *fwd_c_builder_, sequence[t], h_fwd, c_fwd);
    ...
    for (uint32_t i = 0; i < T; ++i) {
        ...
        std::vector<float> h = lstm_cell_step(*bwd_h_builder_, *bwd_c_builder_, sequence[t], h_bwd, c_bwd);
