...
// --- Shared helpers, matching the exact patterns already verified in each component's own e2e test. ---

// Runs a plain (non-recurrent) topology once. x_tc: T-major host vector-of-vectors (real per-position
// data, e.g. BiLstmStepper output) -> converted to ggml's own Layout A flat buffer (ne=[T,C], flat
// index c*T+t) before feeding "x_name"; style fed as-is. Returns the raw output buffer AS-IS (whatever
// layout the topology's own output convention is -- callers interpret it per their own call site, same
// as every test file this whole milestone). `out_T`/`out_C` are written BEFORE returning (never a raw
// ggml_tensor* out-param -- that would dangle once this function's own GraphBuilder/context is
// destroyed, a real use-after-free bug caught here before trusting this helper).
...
// Layout-A raw flat buffer ([T,C], flat=c*T+t) -> T-major host vector-of-vectors.
...
std::unique_ptr<GgufModel> KokoroDriver::load(const std::string& gguf_dir, const std::string& filename) {
    return GgufModel::load(gguf_dir + "/" + filename, backend_);
}

KokoroDriver::KokoroDriver(const std::string& gguf_dir, KokoroConfig cfg, ggml_backend_t backend)
    ...
    decoder_core_model_ = load(gguf_dir, "kokoro_decoder_core.gguf");
    decoder_core_topo_ = parse_topo(*decoder_core_model_);

    sinegen_model_ = load(gguf_dir, "kokoro_sinegen.gguf");
    ...
    generator_model_ = load(gguf_dir, "kokoro_generator.gguf");
    generator_topo_ = parse_topo(*generator_model_);
}

std::vector<float> KokoroDriver::synthesize(const std::vector<int32_t>& input_ids,
                                             const std::vector<float>& ref_s, float speed, uint32_t seed) {
    rng_.seed(seed);
    ...
    const std::vector<float> s_decoder(ref_s.begin(), ref_s.begin() + style_dim);
    const std::vector<float> s_predictor(ref_s.begin() + style_dim, ref_s.begin() + 2 * style_dim);
    ...
    const std::vector<std::vector<float>>& d = x;  // (T_text, 640) -- DurationEncoder's real "d"
    ...
    std::vector<std::vector<float>> top_out;
    {
        ...
        top_out = stepper->run(d);  // T_text x 512
    }
    ...
    const std::vector<uint32_t> pred_dur = predict_durations(duration_logits, speed);
    ...
    std::vector<std::vector<float>> shared_out;
    {
        ...
        shared_out = stepper->run(en);  // T_frames x 512
    }
    ...
    const uint32_t T_f0 = static_cast<uint32_t>(f0_feat.size());  // 2*T_frames
    ...
    const std::vector<float> F0_curve = run_proj(*f0_proj_model_, f0_proj_topo_, f0_feat);  // length T_f0
    ...
    std::vector<float> noise_tc(static_cast<size_t>(dim) * L);  // Layout A ([L,dim])
    ...
    for (uint32_t i = 0; i < pad; ++i) waveform_padded[i] = har_source[pad - i];               // reflect, left
    for (uint32_t i = 0; i < L; ++i) waveform_padded[pad + i] = har_source[i];
    for (uint32_t i = 0; i < pad; ++i) waveform_padded[pad + L + i] = har_source[L - 2 - i];    // reflect, right
