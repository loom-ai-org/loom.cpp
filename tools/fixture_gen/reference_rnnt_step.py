#!/usr/bin/env python3
"""Independent numpy computation of the tiny synthetic plain-RNN-T decode (see rnnt_step_common.py),
writing the encoder output plus the expected token/frame-index sequence for test_rnnt_decoder.cpp to
compare loom::TdtDecoder's plain-RNN-T mode against.

Usage: python3 reference_rnnt_step.py <out_dir>
...
def main() -> None:
    out_dir = Path(sys.argv[1]) if len(sys.argv) > 1 else Path("rnnt_step_ref")
