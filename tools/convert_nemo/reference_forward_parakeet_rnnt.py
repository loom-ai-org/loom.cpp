#!/usr/bin/env python3
"""Independent plain-PyTorch reimplementation of the real Parakeet-RNNT-0.6B forward pass -- mel
frontend, FastConformer encoder (dw_striding subsampling, BIASED throughout, WITH xscale -- see
convert_parakeet_rnnt.py's own module docstring for the confirmed real differences from its TDT
sibling), LSTM prediction network (2 stacked layers), joint network, and PLAIN RNN-T greedy decode (no
duration head/logic at all) -- used as the ground truth test_e2e_parakeet_rnnt.cpp compares loom-engine's
C++ output (encoder + TdtDecoder's new plain-RNN-T mode) against.

Deliberately hand-rolled rather than using nemo_toolkit/transformers (broken in this venv, confirmed in
reference_forward_parakeet_tdt.py already) -- same "avoid the heavier dependency chain" precedent as
every other reference script in this project.
...
from convert_parakeet_rnnt import hparams


def compute_mel_features(waveform: np.ndarray, hp: dict) -> np.ndarray:
    """Same algorithm as reference_forward_parakeet_tdt.py's compute_mel_features (mel_common.py's own
    functions are n_mels-generic; this checkpoint's feat_in=80, unlike parakeet-tdt's 128)."""
    ...
    # Real NeMo's own `get_seq_len`/`normalize_batch` (see mel_common.py's "Real-NeMo gotcha" docstring
    # note) always treats the LAST STFT frame as invalid, even for a full-length, un-padded utterance --
    # mean/var are taken over only the first t_mel-1 frames, and that last frame is zeroed in the output.
    t_mel = log_mel.shape[2]
    valid = log_mel[:, :, : t_mel - 1]
    mean = valid.mean(dim=2, keepdim=True)
    var = ((valid - mean) ** 2).sum(dim=2, keepdim=True) / (valid.shape[2] - 1)
...
def rel_pos_mhsa(x: torch.Tensor, pos_emb: torch.Tensor, state: dict, prefix: str, hp: dict) -> torch.Tensor:
    ...
    p = F.linear(pos_emb, state[f"{prefix}.linear_pos.weight"]).view(n_batch_pos, -1, n_head, head_dim)  # no bias, ever
    p = p.transpose(1, 2)
...
def encoder_forward(mel: np.ndarray, state: dict, hp: dict):
    ...
    x = x * math.sqrt(hp["n_embd"])  # xscaling=True for THIS checkpoint (confirmed real, unlike parakeet-tdt)
...
def lstm_stack_step(last_label: int, h_layers, c_layers, state: dict, hp: dict):
    ...
    for layer in range(hp["pred_rnn_layers"]):
        p = "decoder.prediction.dec_rnn.lstm."
...
def greedy_decode_rnnt(encoder_output: np.ndarray, state: dict, hp: dict, max_symbols: int = 10):
    """Standard RNN-T greedy control flow (NO duration head/logic, unlike TDT): blank always advances
    exactly one frame; non-blank never advances (stays on the same frame, up to max_symbols total
    emissions per frame). Returns (tokens, frame_indices)."""
...
def main() -> None:
    ...
    tokens, frame_indices = greedy_decode_rnnt(encoder_out, state, hp)
