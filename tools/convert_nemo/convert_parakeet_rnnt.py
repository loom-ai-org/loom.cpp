#!/usr/bin/env python3
"""Converts the real NVIDIA nvidia/parakeet-rnnt-0.6b checkpoint (a .nemo file) into loom-engine GGUFs:
one for the FastConformer encoder (mel frontend + encoder), and one pair (h/c) per LSTM layer plus one
for the joint network, for TdtDecoder's new plain-RNN-T mode (durations left empty -- see
tdt_decoder.h/.cpp).

Real hparams/weight-schema confirmed directly against THIS checkpoint's own model_config.yaml and
model_weights.ckpt state dict (not assumed from convert_parakeet_tdt.py's sibling checkpoint, despite the
strong family resemblance -- real, checkpoint-specific differences found and confirmed below, not
copy-pasted blind):
  - Encoder: 24 layers, d_model=1024, n_heads=8, ff_expansion=4 (ff_hidden=4096), conv_kernel_size=9,
    feat_in=80 (NOT 128 like parakeet-tdt-0.6b-v3 -- this checkpoint's mel frontend matches
    Conformer-CTC-small's own 80-mel convention instead), subsampling="dw_striding" (same 3-stage
    depthwise+pointwise structure as parakeet-tdt), xscaling=TRUE (the OPPOSITE of parakeet-tdt's
    xscaling=false -- confirmed against this checkpoint's real config.yaml, not assumed shared with its
    TDT sibling), use_bias unset in the config but confirmed BIASED throughout via the real state dict:
    feed_forward1/2's linear1/linear2, self_attn's linear_q/k/v/out (NOT linear_pos, which never has a
    bias in NeMo's RelPositionMultiHeadAttention regardless of use_bias -- confirmed absent from the real
    state dict), and the conv module's pointwise_conv1/depthwise_conv/pointwise_conv2 -- matching
    Conformer-CTC-small's biased convention, not parakeet-tdt's unbiased one. So this checkpoint is a
    genuine hybrid: parakeet-tdt's dw_striding subsampling structure + Conformer-CTC-small's bias/xscale
    conventions -- confirmed real, not assumed from either sibling alone.
  - Decoder: RNNTDecoder, a real 2-layer stacked nn.LSTM (pred_hidden=640) -- IDENTICAL tensor names/
    shapes to parakeet-tdt's own decoder.prediction.* (confirmed against the real state dict), so
    build_lstm_topology is imported and reused verbatim from convert_parakeet_tdt.py, not reimplemented.
  - Joint: RNNTJoint (NOT RNNTJoint+TDT), joint_hidden=640, activation=relu, num_classes=1024, NO
    duration head at all -- confirmed: config["joint"]["jointnet"] has no "durations" key whatsoever
    (unlike parakeet-tdt's config, which does), and joint.joint_net.2.weight's real shape is (1025, 640)
    == num_classes+1(blank) exactly, no extra columns. Same tensor names as parakeet-tdt's joint, so
    build_joint_topology is also imported and reused verbatim -- the topology JSON doesn't encode the
    output width at all (that comes from the GGUF tensor's own shape), so nothing needs changing there.
...
from convert_parakeet_tdt import build_joint_topology, build_lstm_topology
...
def hparams(config: dict) -> dict:
    ...
    return {
        ...
        "subsampling_num_layers": 3,  # dw_striding, same as parakeet-tdt -- confirmed via real conv.0/2/3/5/6 tensors
        ...
        "durations": [],  # plain RNN-T: no duration head at all (TdtDecoder's new empty-durations mode)
    }
...
def build_encoder_topology(hp: dict) -> dict:
    ...
    def half_step_ff(x, prefix, out):
        # linear2's weight/bias are pre-scaled by 0.5 at conversion time (same fold as Conformer-CTC's
        # own half_step_ff), so this is just LN -> Linear -> Swish -> Linear -> residual-add.
    ...
    # ---- Mel-spectrogram front-end -- identical to convert_conformer_ctc.py's/convert_parakeet_tdt.py's,
    #      parametrized by this checkpoint's own feat_in=80 (Conformer-CTC-small's own convention, NOT
    #      parakeet-tdt's 128). ----
    ...
    #      real state dict (encoder.pre_encode.conv.{0,2,3,5,6}), all biased. ----
    c = hp["subsampling_conv_channels"]
    node("CONV_2D", ["pre_encode.conv0.weight", "mel_input"], ["sub0_raw"],
         {"s0": 2, "s1": 2, "p0": 1, "p1": 1, "d0": 1, "d1": 1})
    bias0 = broadcast_bias_reshape_2d("pre_encode.conv0.bias", c, "conv0_bias_r")
    ...
    for stage in (1, 2):
        ...
        dw_raw = node("CONV_2D_DW", [f"pre_encode.conv{dw_idx}.weight", prev], [f"sub{stage}_dw_raw"],
                      {"s0": 2, "s1": 2, "p0": 1, "p1": 1, "d0": 1, "d1": 1})
        dw_bias = broadcast_bias_reshape_2d(f"pre_encode.conv{dw_idx}.bias", c, f"sub{stage}_dw_bias_r")
        ...
        pw_bias = broadcast_bias_reshape_2d(f"pre_encode.conv{pw_idx}.bias", c, f"sub{stage}_pw_bias_r")
    ...
    # pre_encode.out's weight/bias are pre-scaled by xscale=sqrt(n_embd) at conversion time (xscaling=
    # TRUE for this checkpoint, unlike parakeet-tdt) -- this IS the encoder's running "cur", already xscaled.
    linear("sub_flat", "pre_encode.out.weight", "pre_encode.out.bias", "cur")

    # ---- 24x Conformer layers (biased throughout -- see module docstring) ----
    layer_nodes = []
    ...
    p_flat = node("MUL_MAT", ["blk.{i}.self_attn.linear_pos.weight", "pos_emb_raw"], ["p_flat"])  # no bias, ever
    ...
    node("ADD", ["dw_raw", dw_bias], ["dw_biased"])
    ...
    node("MUL", ["dw_biased", bn_scale], ["bn_scaled"])
...
def main() -> None:
    ...
    # --- Encoder GGUF ---
    w = GGUFWriter(str(out_dir / "parakeet_rnnt_encoder.gguf"), "loom-parakeet-rnnt-encoder")
    w.add_string("loom.architecture", "parakeet_rnnt_encoder")
    ...
    put("pre_encode.out.weight", state["encoder.pre_encode.out.weight"], scale=xscale)
    put("pre_encode.out.bias", state["encoder.pre_encode.out.bias"], scale=xscale)
    ...
    for i in range(hp["n_layers"]):
        ...
        put(f"blk.{i}.feed_forward1.linear2.weight", state[f"{p}.feed_forward1.linear2.weight"], scale=0.5)
        put(f"blk.{i}.feed_forward1.linear2.bias", state[f"{p}.feed_forward1.linear2.bias"], scale=0.5)
        ...
        put(f"blk.{i}.self_attn.linear_pos.weight", state[f"{p}.self_attn.linear_pos.weight"])  # no bias, ever
        ...
        put(f"blk.{i}.feed_forward2.linear2.weight", state[f"{p}.feed_forward2.linear2.weight"], scale=0.5)
        put(f"blk.{i}.feed_forward2.linear2.bias", state[f"{p}.feed_forward2.linear2.bias"], scale=0.5)
    ...
    w.close()
    print(f"wrote {out_dir / 'parakeet_rnnt_encoder.gguf'}")

    # --- LSTM (per-layer h/c) + joint GGUFs -- SAME structure/tensor names as parakeet-tdt's decoder, so
    #     build_lstm_topology/build_joint_topology are imported and reused verbatim (see module docstring). ---
    def write_small(path, topology, tensor_names):
        ww = GGUFWriter(str(path), "loom-parakeet-rnnt-decoder")
        ww.add_string("loom.architecture", "parakeet_rnnt_decoder")
    ...
    for layer in range(hp["pred_rnn_layers"]):
        write_small(out_dir / f"parakeet_rnnt_lstm_h_{layer}.gguf",
                    build_lstm_topology(layer, "h_new", hp["pred_hidden"]), all_decoder_joint_tensor_names)
        write_small(out_dir / f"parakeet_rnnt_lstm_c_{layer}.gguf",
                    build_lstm_topology(layer, "c_new", hp["pred_hidden"]), all_decoder_joint_tensor_names)

    write_small(out_dir / "parakeet_rnnt_joint.gguf", build_joint_topology(hp["n_embd"], hp["pred_hidden"]),
    ...
    print(f"hparams: n_layers={hp['n_layers']} n_embd={hp['n_embd']} pred_hidden={hp['pred_hidden']} "
          f"pred_rnn_layers={hp['pred_rnn_layers']} blank_id={hp['blank_id']} num_classes={hp['num_classes']}")
