"""Converts StyleTTS2 (yl4579/StyleTTS2-LJSpeech) into a SINGLE self-contained loom-engine GGUF file
(`styletts2.gguf`): the 44 topologies `loom::StyleTTS2Driver::synthesize()` uses (CustomAlbert, the
style-diffusion Transformer1d denoiser, bert_encoder, TextEncoder's own CNN+BiLSTM, 3x DurationEncoder
BiLSTM+AdaLayerNorm, the top BiLSTM, duration_proj, F0Ntrain's shared BiLSTM + F0/N AdainResBlk1d stacks
+ projections, the Decoder core, SineGen, the forward STFT, and the Generator) plus the embedded Lua
orchestration script (`model.driver_script`) -- the same one-GGUF-per-model convention already landed
for Whisper/SupertonicTTS/Matcha-TTS/VITS/Kokoro (see BACKLOG.md's dated entries).
`kokoro_stft_inverse`'s equivalent is intentionally NOT included -- confirmed dead weight in
StyleTTS2Driver's own C++ constructor too (loaded, never referenced in synthesize()).

Every piece EXCEPT the style-diffusion sampler is architecturally identical to (and reuses the exact
same build_* functions as) Kokoro's own already-verified pieces -- see convert_styletts2_reused.py's own
docstring for why this reuse is real, not coincidental. This script mirrors
tools/convert_kokoro/convert_kokoro_lua_all.py's own namespacing pattern (distinct weight namespaces per
BiLSTM/AdaLayerNorm/proj instance, since Kokoro's per-module scripts historically wrote each such
instance to its own isolated GGUF file under identical generic weight names) against StyleTTS2's own
real checkpoint weights, plus the one genuinely new piece: convert_styletts2_diffusion.py's
build_diffusion_net.

Usage: python3 convert_styletts2_lua_all.py <epoch_2nd_00100.pth> <out_dir>
"""
import json
import sys
...
from convert_styletts2_diffusion import build_diffusion_net, TopologyBuilder as DiffusionTopologyBuilder, HP as DIFFUSION_HP
...
def main():
    ...
    out = convert_kokoro_albert.build_albert(tb, net["bert"], albert_hp)
    albert_inputs = [
    ...
    # --- style-diffusion Transformer1d denoiser (StyleTTS2's own genuinely new piece, single instance) ---
    tb = DiffusionTopologyBuilder()
    out = build_diffusion_net(tb, net["diffusion"], "module.unet", DIFFUSION_HP)
    diffusion_inputs = [
        {"name": "x_in", "dtype": "f32", "shape": [str(DIFFUSION_HP["channels"])]},
        {"name": "time", "dtype": "f32", "shape": ["1"]},
        {"name": "embedding", "dtype": "f32", "shape": [str(DIFFUSION_HP["context_embedding_features"]), "$n_tokens"]},
        {"name": "attn_mask", "dtype": "f32", "shape": ["$n_tokens", "$n_tokens"]},
    ]
    add("diffusion", tb.topology(diffusion_inputs, out), tb.weights)

    # --- bert_encoder ---
    tb = convert_kokoro_bert_encoder.TopologyBuilder()
    out = convert_kokoro_bert_encoder.build_bert_encoder(tb, net["bert_encoder"], "module")
    bert_encoder_inputs = [{"name": "x", "dtype": "f32", "shape": ["768", "$n_tokens"]}]
    add("bert_encoder", tb.topology(bert_encoder_inputs, out), tb.weights)

    # --- TextEncoder: CNN + its own BiLSTM ---
    te_sd = net["text_encoder"]
    ...
    # --- Decoder core, SineGen, forward STFT, Generator (all real weights under net["decoder"]) ---
    gen_sd = net["decoder"]
    ...
    driver_script_path = Path(__file__).parent / "styletts2_driver.lua"

    w = GGUFWriter(str(out_dir / "styletts2.gguf"), "loom-styletts2")
    ...
    w.close()
    print(f"wrote {out_dir / 'styletts2.gguf'}, {len(topologies)} topologies, {len(merged_weights)} weights")
