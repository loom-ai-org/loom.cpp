...
function synthesize(inputs)
    ...
    local txt_emb_ta = loom.run_subgraph("ttl_text", {n_tokens = t_text, n_past = 0}, { txt_ids = inputs.txt_ids, stl_emb_ttl_cb = inputs.style_ttl })
    local txt_emb_cb = {}
    for t = 0, t_text - 1 do
    ...
    for step = 0, inputs.n_steps - 1 do
        ...
        local v = loom.run_subgraph("vfe", {n_tokens = t_lat, n_past = 0}, {
            z_t = z,
