...
int main() {
    const char* dir_env = std::getenv("LOOM_VITS_MIL_DIR");
    const char* ref_dir_env = std::getenv("LOOM_VITS_FLOW_VOCODER_REF_DIR");
    ...
    loom::GraphBuilder::BuildResult r = builder.build(Tp, 0);
