// Numerical-correctness check for the MIL-traced "albert_bert_encoder" combined topology
// (export_kokoro_mil.py's AlbertBertEncoderWrapper, part of kokoro_mil.gguf) against a hand-rolled
// pure-PyTorch reference (reference_forward_kokoro_albert_bert_encoder_mil.py). Mirrors
// test_e2e_kokoro_albert.cpp's own npy-reading/comparison shape, but against the combined MIL topology
// (single `tokens` input, no separate `positions`/`attn_mask` -- see AlbertBertEncoderWrapper's own
// docstring) and its (T,512) time-major output convention (NOT (512,T) -- no final transpose, see the
// reference script's own docstring for why). Skips cleanly if the GGUF/reference files aren't present.
...
int main() {
    const char* gguf_env = loom_test::fixture_env("LOOM_KOKORO_MIL_GGUF");
    const char* ref_dir_env = loom_test::fixture_env("LOOM_KOKORO_MIL_ALBERT_REF_DIR");
    if (gguf_env == nullptr || ref_dir_env == nullptr) {
        std::fprintf(stderr, "skipping: set LOOM_KOKORO_MIL_GGUF (kokoro_mil.gguf, produced by "
                              "export_kokoro_mil.py) and LOOM_KOKORO_MIL_ALBERT_REF_DIR "
                              "(ref_albert_bert_encoder_*.npy, produced by "
                              "reference_forward_kokoro_albert_bert_encoder_mil.py) to run this check\n");
        return 77;
    }
    ...
    std::vector<int32_t> tokens = read_npy_i32(ref_dir + "/ref_albert_bert_encoder_tokens.npy", tok_shape);
    std::vector<float> ref_out = read_npy_f32(ref_dir + "/ref_albert_bert_encoder_out.npy", out_shape);
    LOOM_CHECK(tok_shape.size() == 1);
    LOOM_CHECK(out_shape.size() == 2); // (T,512), time-major -- see module docstring
