// Numerical-correctness check for Kokoro Generator's `AdaINResBlock1` (istftnet.py) -- the OTHER
// "AdaIN*"-family resblock (distinct from predictor.F0/N's AdainResBlk1d): 3x (AdaIN1d+Snake+dilated
// conv1) -> (AdaIN1d+Snake+conv2) -> residual-add stages, no shortcut/upsample at all. Verified against a
// hand-rolled pure-PyTorch reference on a small SYNTHETIC instance (checkpoint-independent structural
// verification, same precedent as VITS's own test_hifigan_generator). Fully deterministic exact-match
...
int main() {
    ...
    const char* ref_dir_env = loom_test::fixture_env("LOOM_KOKORO_ADAINRESBLOCK1_REF_DIR");
    if (dir_env == nullptr || ref_dir_env == nullptr) {
        std::fprintf(stderr, "skipping: set LOOM_KOKORO_DIR (kokoro_adainresblock1.gguf) and "
                              "LOOM_KOKORO_ADAINRESBLOCK1_REF_DIR (ref_adainresblock1_*.npy) to run this "
                              "numerical check\n");
        return 77;
    }
    ...
    constexpr uint32_t kStyleDim = 8;

    std::vector<int64_t> x_shape, style_shape, out_shape;
    std::vector<float> x = read_npy_f32(ref_dir + "/ref_adainresblock1_x.npy", x_shape);
    std::vector<float> style = read_npy_f32(ref_dir + "/ref_adainresblock1_style.npy", style_shape);
    std::vector<float> ref_out = read_npy_f32(ref_dir + "/ref_adainresblock1_out.npy", out_shape);
    ...
    auto model = loom::GgufModel::load(dir + "/kokoro_adainresblock1.gguf", backend.get());
