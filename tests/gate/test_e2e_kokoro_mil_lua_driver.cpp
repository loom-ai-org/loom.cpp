...
int main() {
    const char* gguf_mil_env = loom_test::fixture_env("LOOM_KOKORO_MIL_GGUF");
    ...
    std::vector<float> lua_wav;
    {
        ...
        // each half of the same step (recurrent.py::_lstm_cell_topology). 27 until
        ...
        // (ADR-031's follow-up).
        LOOM_CHECK(model_mil->topology_names().size() == 28);
        ...
        // reshaped by a binding rather than by a build -- and until this loop existed every test in the
        // suite called `infer` exactly once per process, so that state was covered by nothing. The bug
        // that put this here (Retro-045) corrupted the heap on the second synthesis and read a
        // frame-expanded sequence out of a store the driver had just rewritten; a single-call test
        // cannot see either. Same inputs, so same seed and same answer: any difference is stale state.
