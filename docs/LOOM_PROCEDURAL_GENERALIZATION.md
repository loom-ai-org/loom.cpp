...
# Modular Profile: traces each declared submodule independently for low-VRAM edge-device execution
# (a distinct entry point -- ModularExportSpec / export_modular -- not a `profile=` string, below)
```
...
### 6.3 The Modular-Export Path (`ModularExportSpec`)
...
* **Declared, not inferred**: the caller declares the boundary (`ModularExportSpec`: prefix/aux/per-layer/
...
* **No silent fallback**: unlike the retired atomic path, there is no monolithic downgrade-on-failure --a
  modular export either succeeds against the declared boundary or raises.
