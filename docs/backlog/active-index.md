...
| **P5 family 5 — Paraformer's VOCABULARY, the one piece left** | The exporter half SHIPPED 2026-09-16: `paraformer_export.py` produces a GGUF whose ids detokenize, through FunASR's own postprocess, to a character-for-character identical transcript on both a Chinese and an English clip. What it deliberately does NOT write is a vocabulary, so `model.tokenizer` is None and the file returns ids. The rule is real and has no reader: merge `@@`-suffixed subword continuations, space Latin words, join CJK bare, skip `<s>/</s>/<unk>/<OOV>`. It cannot be folded into an existing family — `@@` marks "continues into the NEXT piece" where SentencePiece's `▁` and WordPiece's `##` mark word START, and the same piece string appears in both roles. **The cheap shape is a per-piece `space_after` flag** computed at export (`not piece.endswith("@@")` and the piece is Latin), decode being `join(piece.replace("@@","") + (" " if space_after))`. That is [ADR-033](../adrs/adr-033-a-decode-only-table-is-still-a-vocabulary-family.md)'s shape one family over, and it is where family 5 stops being free of engine work → [Epic-03 §2](../epics/epic-03-model-coverage.md) |
