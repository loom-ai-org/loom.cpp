...
date: 2026-07-26
domain: exporter
tags: [mil, tracing, model-coverage, exporter-gaps, numerical-verification]
---

# Retro-013: Retrofitting Eight Bespoke Converters onto the MIL Exporter

## The Issue

Eight models — Qwen3-0.6B-Base, NeMo Conformer-CTC, Parakeet-TDT, Parakeet-RNNT, VITS (piper), Kokoro,
StyleTTS2, Matcha-TTS and SupertonicTTS — each had a hand-written `tools/convert_*` script, roughly
14,000 lines across ten directories. Every one had to become a trace of the **real** upstream module
through the MIL compiler, numerically verified, before the bespoke scripts could be retired.

## Root Cause Analysis

The retrofits were where the exporter's actual generality got tested, and each model found gaps the
previous ones had not. The recurring shapes, worth knowing before adding a ninth model:

* **Missing primitives and unwired op-map entries** — `logical_not`, `stack`, `sqrt`, `log`. Usually
  composable from what already existed; a genuinely new primitive was the exception, not the rule.
  Relative-position attention decomposed under tracing into ordinary matmul/pad/reshape/gather, and
  needed **no new attention primitive at all**.
* **Symbolic-shape inference collapsing to one axis.** `get_var_info` collapsed every symbolic dim to
  the bare string `n_tokens` — correct for every model up to Qwen3, and genuinely wrong for
  Conformer-CTC, whose mel front end introduces a *second, derived* dynamic quantity (STFT frame count,
  `floor(n_tokens/160)+1`) that also presents as a bare opaque symbol. Fixed by deriving the expression
  through a bounded backward walk rather than guessing.
* **MIL conventions honoured only in their concrete-integer case** — `slice_by_index` ignoring
  `begin_mask`/`end_mask`, negative bounds not normalised against a *symbolic* dim, `reduce_sum` always
  fully reducing. Each silently produced a wrong shape rather than an error.
* **A value that does not exist yet.** `gather(shape(x), axis)` has no value until
  `ggml_backend_graph_compute()` runs, which is strictly *after* the op that wanted to read it. The
  fallback fired silently no matter what was fixed upstream. Architectural, not a shape-string bug.
* **Memory-safety hazards from unbounded shapes** — a `torch.full` sized off a non-constant shape
  pre-allocated `[4096]^rank`, i.e. 256 GiB at rank 3.

## Resolution & Lesson Learned

All eight retrofitted and numerically verified; the bespoke converters are gone (see
[ADR-004](../adrs/adr-004-mil-as-the-single-export-path.md)).

* **Actionable takeaway 1 — informative errors are a prerequisite, not a polish step.**
  `RESHAPE`/`VIEW`/`MUL_MAT` gained real shape-mismatch messages in place of raw `GGML_ASSERT` aborts,
  and that is what made every subsequent bug *findable at all* instead of a bare crash with no context.
* **Actionable takeaway 2 — trace the real upstream module, never a re-implementation.** The older
  plain-PyTorch POC needed a custom torch op; tracing the real `EncDecCTCModelBPE` needed none.
* **Actionable takeaway 3 — a heuristic that is "documented, deliberate and correct for every model so
  far" is a landmine with a date on it.** Two of the hardest bugs here were exactly that.
* **Actionable takeaway 4 — re-verify the already-done models after every fix.** Qwen3 was re-verified
  byte-for-byte identical throughout, which is what made each gap safe to close.
