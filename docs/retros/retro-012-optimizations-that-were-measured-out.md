...
| **cache-blocking the reduction in `tinyBLAS_F32_ARMV6`**, so the patch panel stops being re-read once per 4-column tile | **wins on two of seven shapes and loses on five** — 2200 k=7 is 1.55x and k=3 is 1.16x, everything else 0.73-0.99x, for 1.76 s of a 72 s synthesis. An L1-derived gate (block when the 4x4 tile's eight operand strips exceed it, `k > 512`) captures 1.58 s and classifies six of seven correctly. Not taken because it reorders the reduction — not bit-identical — and needs accumulate variants of every tile kernel. **Two harness lessons came with it**: the first sweep reported 1.34x on a shape that re-measures at 0.98x twice, because running several `kc` arms in sequence changes the cache state the next one starts in — re-measure the FLAT arm after the swept ones and print the drift; and the 16-21 s this was scoped against came from dividing MACs by a roofline measured at another shape. `scripts/bench40.c` |
