...
| **a fixed column-tile-outer order in the ARMv6 q4_0 GEMM**, tuned on a VITS convolution shape | **conformer-ctc 5% SLOWER on the board** while the microbenchmark said 1.73x. The order decides which operand is re-read per tile, and the right answer is per shape: at a convolution's m=384 n=276 the activation panel is 275 KB and the weights 202 KB, so stream the weights; at a 17-token transformer matmul the panel is **13.5 KB** and fits in L1, so re-reading 324 KB of weights eight times to avoid re-reading 13.5 KB is exactly backwards. Now chosen per call from the two panel sizes -- **though not until 5f7c18b**: the decision had been written into `tinyBLAS_Q0_ARM`, the 64-bit class, instead of its ARMv6 copy, so `col_outer` stayed `false` for every call and aarch64 did not compile at all. Five days, and only loom-py's macOS job could see it. **The microbenchmark was right and the generalisation was wrong** — Epic-08 §6.6 |
| **Q8_0 instead of Q4_0 to get integer arithmetic into VITS's convolutions on ARMv6** | **0%, and it cannot be otherwise**: `ggml_conv_2d_direct_packed` dequantizes the kernel to F32 once per call and every lowering below that is the F32 one, so the quantization type never reaches the inner loop. ABBA on a Pi Zero W: Q4_0 **80.6x** real time, Q8_0 **81.1x**, F32 **81.7x**. [Epic-08 §6.5](../epics/epic-08-packaging-and-release.md) |
| ~~`GGML_CPU_DISABLE_CONV_HEURISTICS=1` on ARMv6~~ **— REVERSED, see the note under this table** | **1.63x SLOWER** — 416.1 / 416.9 s against 256.3 / 264.1 s on a Pi Zero W. The budget really is wrong there (`sysconf` reports 0 for every level, so it takes the floor), and the im2col + F32 `MUL_MAT` fallback is worse anyway. Epic-08 §6.5 |
| **walking channels on the outside in `conv_2d`'s permute-back**, so the write is contiguous and the strided side is the READ | **0.24 s SLOWER end to end** — 68.98 / 69.00 s against 69.22 / 69.24 s, brackets 0.02 s wide — where the phase measured alone is **3.2x faster** and saves 0.44 s across a synthesis (69.6 -> 21.7 ms at the largest shape, `scripts/bench42.c`). Bit-identical at one and four threads, correct, and not worth having. The `CONV_2D` buckets do improve by 239 ms; something outside them gets slower by more. **The third phase in this op whose isolated number does not survive contact with the graph, and the second SIGN change** — the gather understated by 5x, this overstates past zero. Also measured: letting the GEMM write straight into `dst` with `ldc = dst_w*dst_h`, which ggml-0004's header proposes and `defer` uses, is worse than either (43.6 ms against 21.7) because the bias pass that remains must read-modify-write the destination |
| **the ARMv6 q4_0 x q8_0 convolution GEMM** (`ggml-0019`), which WON 5.65x when it was written | **0.90x now — declining it is 1.107x** (VITS 20.92x -> 18.89x real time per second of audio). The 5.65x was against ggml's untiled `vec_dot` F32 loop at 29.7 MMAC/s; that path is now `tinyBLAS_F32_ARMV6` plus cache blocking at 178-262. **The kernel it beat no longer exists.** Kept opt-in behind `GGML_CPU_ENABLE_ARMV6_CONV_QGEMM`. **And it had been ON BY ACCIDENT**: it was unreachable for VITS until the patch cap moved 512 KB -> 64 KB for cache reasons, which also made the Q8_0 staging fit — so that change did two things and the 1.061x credited to it netted a win against this loss. This register's own rule caught it: re-run an entry before citing it if the incumbent has changed. **The rule applies to entries that WON, not only to the ones that lost.** Epic-08 §6.8 |
| ~~**cache-blocking the reduction in `tinyBLAS_F32_ARMV6`**~~ **— SHIPPED, gated** | It wins on two of seven shapes and loses on five, which is why it sat here; what changed is that the gate stopped being fitted. A 4x4 tile keeps eight operand strips live, so they stop fitting the L1 at `k > L1/32 = 512` — derived, not tuned — and that classifies six of the seven correctly, leaving one 1.16x on the table rather than widening to catch it. **1.017x end to end**, and the first non-bit-identical change on this target (rel-RMS 6.9e-4, a reordered sum; a lowering change gives 0.19). Epic-08 §6.8. **The harness lesson stays**: the first sweep reported 1.34x on a shape that re-measures at 0.98x twice, because running several `kc` arms in sequence changes the cache state the next one starts in — re-measure the flat arm AFTER the swept ones and print the drift. `scripts/bench40.c` |
| **a gather instead of the scatter-add in `conv_transpose_1d`'s overlap-add** | **22 ms of a 72 s synthesis, 0.03%** — 1.18x / 1.11x / 1.03x on the three nodes, and that is the OPTIMISTIC arm: `gemm` is recomputed per tile, so a correct gather has to carry the previous tile's last row across the boundary, which the benched version skips. The whole non-GEMM phase of that op is 545 ms (overlap-add 389, transposes 120, kernel repack 35) and the other two are structural — the GEMM needs position-major and the tensor is channel-major, and ggml has no per-node persistent scratch to cache a constant repack in. `scripts/bench39.c` |
| **a phase-major ("a trous") layout for the ARMv6 direct sweep**, to make each channel's `k` taps contiguous | **loses on five of VITS's six OC=32 convolutions and wins 1.08x on the sixth** — 261 ms, 0.36% of a synthesis. The rate really does track the tap window's byte span (207.7 MMAC/s at 8 B down to 107.9 at 288 B), which is why this looked good, but the DENSE CEILING is only 165-191 MMAC/s: a layout fix cannot reach the GEMM's rate on this core, and the de-interleave costs 138-396 ms per call. `scripts/bench38.c` bounds it without porting it — arm A the shipped tile, arm B the same tile dense, arm C the transform, build iff A > B+C. Epic-08 §6.8 |
| **routing VITS's OC=32 bucket to im2col + GEMM** now that P7.1 recut the patch cap | **12% SLOWER** — 74.58 s against 83.58 s with `GGML_CPU_CONV1D_BUDGET=0`, and 80.09 s peeling only the 20 and 28 KB convolutions off. The direct sweep is worth **1.12x** on that bucket, not the 5% previously recorded. Worth re-testing because the arm it had lost to had since got faster; it still loses |
...
**One entry above has since reversed, and it is left in place on purpose.** The ARMv6
`GGML_CPU_DISABLE_CONV_HEURISTICS=1` row measured 1.63x SLOWER against ggml's untiled F32 GEMM. With
`tinyBLAS_F32_ARMV6` in place that same switch is **1.27x FASTER** (VITS 130.6 -> 103.0 s), because
the thing it switches TO got three times quicker. Both measurements are correct about the kernel they
were taken against, which is the point worth carrying: **a register in this file records that an idea
lost to a specific incumbent, not that it is wrong forever.** Re-run an entry before citing it if the
incumbent has changed. Epic-08 §6.8.
