...
| `GGML_CPU_DISABLE_CONV_HEURISTICS=1` on ARMv6, on the theory that the 512 KB cache budget is a 32x overestimate on a board with 16 KB of D-cache | **1.63x SLOWER** — 416.1 / 416.9 s against 256.3 / 264.1 s on a Pi Zero W. The budget really is wrong there (`sysconf` reports 0 for every level, so it takes the floor), and the im2col + F32 `MUL_MAT` fallback is worse anyway. Epic-08 §6.5 |
